...
class StandardCypherTest extends LazyLogging {
  ...
  def testStandardTck(): java.util.Collection[DynamicTest] = {
    ...
    val dynamicTests = allTckScenarios.slice(skip_duration, skip_duration + 100) map { scenario =>
    ...
    value match {
      case node: TestNode =>
        CypherNode(
          ...
          CypherPropertyMap(node.props.map {
            case (propName, lynxValue) => (propName.value, lynxValue2CypherValue(lynxValue))
          })
        )
      case relation: TestRelationship =>
        CypherRelationship(
          relation.relationType.map(_.value).getOrElse(""),
          CypherPropertyMap(relation.props.map {
            case (propName, lynxValue) => (propName.value, lynxValue2CypherValue(lynxValue))
          })
        )
