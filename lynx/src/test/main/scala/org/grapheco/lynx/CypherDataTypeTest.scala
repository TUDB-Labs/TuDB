...
class CypherDataTypeTest extends TestBase {
  ...
  def testArrayTypeProperty(): Unit = {
    runOnDemoGraph(
      "create (n:person{name:'xx', arr1:[1,2,3], arr2:['abc','df'], arr3:[1.5,2.0], arr4:[true,false], arr5:[]}) return n"
    )
    val r = runOnDemoGraph(
      "match(n:person{name:'xx'}) return n.arr1, n.arr2, n.arr3, n.arr4, n.arr5"
    ).records().next()

    Assert.assertArrayEquals(
      Array[Long](1, 2, 3),
      r.get("n.arr1").get.asInstanceOf[LynxList].value.map(x => x.value.asInstanceOf[Long]).toArray
    )
    val arr2 = r
      .get("n.arr2")
      .get
      .asInstanceOf[LynxList]
      .value
      .map(x => x.value.asInstanceOf[String])
      .toArray
    ...
    val arr3 = r
      ...
      .asInstanceOf[LynxList]
      .value
      .map(x => x.value.asInstanceOf[Double])
      .toArray
    ...
    Assert.assertArrayEquals(
      ...
      r.get("n.arr4")
        ...
        .asInstanceOf[LynxList]
        .value
        .map(x => x.value.asInstanceOf[Boolean])
        .toArray
    )
  ...
  def testDateTypeProperty(): Unit = {
    ...
    Assert.assertEquals(
      LocalDate.parse("2015-02-01", DateTimeFormatter.ofPattern("yyyy-MM-dd")),
      r1.get("n.born").get.asInstanceOf[LynxDate].value
    )
    val r2 = runOnDemoGraph("match (n:person{name:'date2'}) return n.born").records().next()
    Assert.assertEquals(
      LocalDate.parse("2015-02-02", DateTimeFormatter.ofPattern("yyyy-MM-dd")),
  ...
  def testDateTimeTypeProperty(): Unit = {
    runOnDemoGraph(
      "CREATE (Keanu:person {name:'datetime1',born:datetime('1984-10-11T12:31:14.123456789Z')})"
    )
    runOnDemoGraph(
      "CREATE (Keanu:person {name:'datetime2',born:datetime('1984-10-11T12:31:14.123456789+01:00')})"
    )

    val r1 = runOnDemoGraph("match (n:person{name:'datetime1'}) return n.born").records().next()
    Assert.assertEquals(
      ZonedDateTime.parse("1984-10-11T12:31:14.123456789Z"),
    ...
    Assert.assertEquals(
      ZonedDateTime.parse("1984-10-11T12:31:14.123456789+01:00"),
  ...
  def testLocalDateTimeTypeProperty(): Unit = {
    runOnDemoGraph(
      "CREATE (Keanu:person {name:'localdatetime1',born:localdatetime('2015-07-21T21:40:32.142')})"
    )
    val r1 =
      runOnDemoGraph("match (n:person{name:'localdatetime1'}) return n.born").records().next()
    Assert.assertEquals(
      LocalDateTime.parse("2015-07-21T21:40:32.142"),
  ...
  def testTimeTypeProperty(): Unit = {
    ...
    Assert.assertEquals(
      OffsetTime.parse("21:40:32.142+01:00"),
  ...
  def testDurationType(): Unit = {
    runOnDemoGraph(
      "CREATE (Keanu:person {name:'time1', works: duration('P14DT16H12M'), history: duration({years: 10.2, months: 5, days: 14, hours:16, minutes: 12, seconds: 1, milliseconds: 123, microseconds: 456, nanoseconds: 789})})"
    )
    val r1 =
      runOnDemoGraph("match (n:person{name:'time1'}) return n.works, n.history").records().next()
