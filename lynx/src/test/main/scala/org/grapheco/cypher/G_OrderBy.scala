...
class G_OrderBy extends TestBase {
  ...
  val n1 = TestNode(
    ...
    Map(
      ...
      LynxPropertyKey("age") -> LynxValue(34),
      LynxPropertyKey("length") -> LynxValue(170)
    )
  )
  val n2 = TestNode(
    TestId(2),
    Seq.empty,
    Map(LynxPropertyKey("name") -> LynxValue("B"), LynxPropertyKey("age") -> LynxValue(36))
  )
  val n3 = TestNode(
    TestId(3),
    Seq.empty,
    Map(
      LynxPropertyKey("name") -> LynxValue("C"),
      LynxPropertyKey("age") -> LynxValue(32),
      LynxPropertyKey("length") -> LynxValue(180)
    )
  )
  ...
  val r1 = TestRelationship(
    ...
    TestId(1),
    TestId(2),
    Option(LynxRelationshipType("BLOCKS")),
    Map.empty
  )
  ...
  def orderingNull(): Unit = {
    ...
    Assert.assertEquals(
      List(170, "A", 34),
      List(res(0)("n.length"), res(0)("n.name"), res(0)("n.age")).map(f =>
        f.asInstanceOf[LynxValue].value
      )
    )
    Assert.assertEquals(
      List(180, "C", 32),
      List(res(1)("n.length"), res(1)("n.name"), res(1)("n.age")).map(f =>
        f.asInstanceOf[LynxValue].value
      )
    )
    Assert.assertEquals(
      List(null, "B", 36),
      List(res(2)("n.length"), res(2)("n.name"), res(2)("n.age")).map(f =>
        f.asInstanceOf[LynxValue].value
      )
    )
  }
  ...
  def orderingInAWITHClause(): Unit = {
    ...
    Assert.assertEquals(
      List("C", "A", "B").map(LynxValue(_)),
      res(0)("names").asInstanceOf[LynxValue].value
    )
  }
}
