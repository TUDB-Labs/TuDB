...
class P_Call extends TestBase {
  ...
  val n1 = TestNode(
    TestId(1),
    Seq(LynxNodeLabel("Person"), LynxNodeLabel("Child")),
    Map(LynxPropertyKey("name") -> LynxValue("Alice"), LynxPropertyKey("age") -> LynxValue(20))
  )
  val n2 = TestNode(
    TestId(1),
    Seq(LynxNodeLabel("Person")),
    Map(LynxPropertyKey("name") -> LynxValue("Dora"), LynxPropertyKey("age") -> LynxValue(30))
  )
  val n3 = TestNode(
    TestId(1),
    Seq(LynxNodeLabel("Person"), LynxNodeLabel("Parent")),
    Map(LynxPropertyKey("name") -> LynxValue("Charlie"), LynxPropertyKey("age") -> LynxValue(65))
  )
  val n4 = TestNode(
    TestId(1),
    Seq(LynxNodeLabel("Person")),
    Map(LynxPropertyKey("name") -> LynxValue("Bob"), LynxPropertyKey("age") -> LynxValue(27))
  )
  ...
  val r1 = TestRelationship(
    ...
    TestId(1),
    TestId(3),
    Option(LynxRelationshipType("CHILD_OF")),
    Map.empty
  )
  val r2 = TestRelationship(
    ...
    TestId(1),
    TestId(4),
    Option(LynxRelationshipType("FRIEND_OF")),
    Map.empty
  )
  @Before
  def init(): Unit = {
