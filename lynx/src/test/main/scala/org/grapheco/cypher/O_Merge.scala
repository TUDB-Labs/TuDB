...
class O_Merge extends TestBase {
  ...
  val n1 = TestNode(
    ...
    Map(
      LynxPropertyKey("name") -> LynxValue("Charlie Sheen"),
      LynxPropertyKey("bornIn") -> LynxValue("New York"),
      LynxPropertyKey("chauffeurName") -> LynxValue("John Brown")
    )
  )
  val n2 = TestNode(
    TestId(2),
    Seq(LynxNodeLabel("person")),
    Map(
      LynxPropertyKey("name") -> LynxValue("Oliver Stone"),
      LynxPropertyKey("bornIn") -> LynxValue("New York"),
      LynxPropertyKey("chauffeurName") -> LynxValue("Bill White")
    )
  )
  val n3 = TestNode(
    TestId(3),
    Seq(LynxNodeLabel("person")),
    Map(
      LynxPropertyKey("name") -> LynxValue("Michael Douglas"),
      LynxPropertyKey("bornIn") -> LynxValue("New Jersey"),
      LynxPropertyKey("chauffeurName") -> LynxValue("John Brown")
    )
  )
  val n4 = TestNode(
    ...
    Seq(LynxNodeLabel("person")),
    Map(
      LynxPropertyKey("name") -> LynxValue("Martin Sheen"),
      LynxPropertyKey("bornIn") -> LynxValue("Ohio"),
      LynxPropertyKey("chauffeurName") -> LynxValue("Bob Brown")
    )
  )
  val n5 = TestNode(
    TestId(5),
    Seq(LynxNodeLabel("person")),
    Map(
      LynxPropertyKey("name") -> LynxValue("Rob Reiner"),
      LynxPropertyKey("bornIn") -> LynxValue("New York"),
      LynxPropertyKey("chauffeurName") -> LynxValue("John Brown")
    )
  )
  ...
  val r2 = TestRelationship(
    ...
    TestId(1),
    TestId(4),
    Option(LynxRelationshipType("FATHER")),
    Map.empty
  )
  ...
  def init(): Unit = {
    ...
    relationsInput.append(
      (
        "r8",
        RelationshipInput(
          Seq(r8.relationType.get),
          Seq.empty,
          StoredNodeInputRef(r8.startNodeId),
          StoredNodeInputRef(r8.endNodeId)
        )
      )
    )
  ...
  def testMergeSingleNodeWithALabel(): Unit = {
    ...
    Assert.assertEquals(nodeNum + 1, all_nodes.length)
    Assert.assertEquals(
      TestNode(TestId(nodeNum + 1), Seq(LynxNodeLabel("Critic")), Map.empty),
      records.head("robert").asInstanceOf[LynxNode]
    )
    Assert.assertEquals(
      List("Critic"),
      records
  ...
  def testMergeSingleNodeWithProperty(): Unit = {
    ...
    Assert.assertEquals(
      TestNode(
        ...
        Seq.empty,
        Seq(
          LynxPropertyKey("name") -> LynxString("Charlie Sheen"),
          LynxPropertyKey("age") -> LynxInteger(10)
        ).toMap
      ),
      records.head("charlie")
    )
  ...
  def testMergeSingleNodeDerivedFromExistingNodeProperty(): Unit = {
    ...
    Assert.assertEquals(
      Seq("New York", "New Jersey", "Ohio"),
      records2
        .map(f => f("n").asInstanceOf[LynxNode].property(LynxPropertyKey("name")).get.value)
        .toSeq
    )
  }
  ...
  @Test
  def testMergeOnARelationshipBetweenAnExistingNodeAndAMergedNodeDerivedFromANodeProperty(
    ): Unit = {
    val relNum = relationsInput.size
    val nodesNum = nodesInput.size
