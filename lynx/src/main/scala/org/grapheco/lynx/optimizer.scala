...
object JoinReferenceRule extends PhysicalPlanOptimizerRule {
  def checkNodeReference(
      pattern: NodePattern
    ): (Seq[((LogicalVariable, PropertyKeyName), Expression)], Option[NodePattern]) = {
      ...
      val newPattern = {
        if (filter2.nonEmpty) {
          NodePattern(
            variable,
            pattern.labels,
            Option(MapExpression(filter2)(properties.get.position)),
            pattern.baseNode
          )(pattern.position)
  ...
  def checkExpandPath(
      pe: PPTExpandPath,
      ppc: PhysicalPlannerContext
    ): (PPTNode, Seq[((LogicalVariable, PropertyKeyName), Expression)]) = {
    pe.children match {
      case Seq(pr @ PPTRelationshipScan(rel, leftPattern, rightPattern)) => {
        ...
        val res = (leftChecked._2, rightChecked._2) match {
          ...
          case (value1, value2) =>
            PPTRelationshipScan(rel, leftChecked._2.get, rightChecked._2.get)(ppc)
        }
    ...
    if (referenceProperty.nonEmpty) {
      referenceProperty.length match {
        case 1 => {
          ...
          val filter =
            Equals(Property(ksv._1._1, ksv._1._2)(ksv._1._1.position), ksv._2)(ksv._1._1.position)
        ...
        case _ => {
          ...
          referenceProperty.foreach(f => {
            val filter =
              Equals(Property(f._1._1, f._1._2)(f._1._1.position), f._2)(f._1._1.position)
            setExprs.add(filter)
          })
          PPTJoin(
            Option(Ands(setExprs.toSet)(referenceProperty.head._1._1.position)),
            isSingleMatch
          )(table1, table2, ppc)
        }
      }
