...
object IndexFactory extends StrictLogging {
  ...
  def newIndex(indexUri: String): IndexServer = {
    if (null == indexUri || indexUri.isEmpty || !indexUri.contains("://") || !indexUri.startsWith("tudb")) {
      new EmptyIndexServerImpl(Map.empty)
    } else {
      val url = new URI(indexUri)
      val params = if (url.getQuery == null) Map.empty[String, String] else url.getQuery.split("&")
